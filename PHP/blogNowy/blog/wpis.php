<?php
	...
	function new_post($blog_dir, $text, $date, $time) {
		...
		$block_file = fopen("post_blocker", "w") or die("Nie można zablokować sekcji krytycznej.");
		$lock = flock($block_file, LOCK_EX);
		$unicode = 0;
	...
	if (trim($username) == '' || trim($password) == '' || trim($post) == '') {
		header("Location: http://student.agh.edu.pl/~polarczy/blog/error.php?err=2");
	}
	else {
		$current_dir = getcwd();
		...
		if (!($folder = verify_password($current_dir, $username, $password))) 
			header("Location: http://student.agh.edu.pl/~polarczy/blog/error.php?err=1");
		else {
			$post_file = new_post($folder, $post, $date, $time);
			add_files($post_file, 'fileToUpload', 'files');
			header("Location: http://student.agh.edu.pl/~polarczy/blog/blog.php?nazwa=" . basename($folder));
		}
	}
?>
